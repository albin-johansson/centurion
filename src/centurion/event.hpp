...
/// An event dispatcher wrapper around a `cen::event` instance.
///
/// This class is an attempt to simplify handling events in applications, usually you'd check
/// for all of the events that you'd be interested in using `cen::event::try_get()` in a long
...
/// single call to `event_dispatcher::poll()`.
///
...
///
/// \tparam E the list of events to "subscribe" to. Events that are not present in this list
/// are ignored.
template <typename... E>
