 ...
 * requires is that the type provides a static function that returns a `czstring`.
 ...
 *
 * \details The signature of the callable should be equivalent to `void(UserData*,
 * czstring, Hint::value_type, Hint::value_type)`.
 *
    ...
    -> hint_callback<Hint, UserData>
{
  const auto wrapper = [](void* erased,
                          const czstring name,
                          const czstring oldValue,
                          const czstring newValue) {
    Callable callable;
