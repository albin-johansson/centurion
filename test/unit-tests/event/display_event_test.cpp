...
{
  cen::display_event event;
  event.set_time(1'337);
  event.set_index(123);
  event.set_event_id(cen::display_event_id::connected);
  event.set_data_1(5);

  const auto converted = cen::as_sdl_event(event);
  ASSERT_EQ(SDL_DISPLAYEVENT, converted.display.type);
  ASSERT_EQ(1'337, converted.display.timestamp);
  ASSERT_EQ(123, converted.display.display);
