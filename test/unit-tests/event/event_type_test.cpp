...
TEST(EventType, Values)
{
  ...
  ASSERT_EQ(SDL_QUIT, type::quit);
  ASSERT_EQ(SDL_APP_TERMINATING, type::app_terminating);
  ASSERT_EQ(SDL_APP_LOWMEMORY, type::app_low_memory);
  ASSERT_EQ(SDL_APP_WILLENTERBACKGROUND, type::app_will_enter_background);
  ASSERT_EQ(SDL_APP_DIDENTERBACKGROUND, type::app_did_enter_background);
  ASSERT_EQ(SDL_APP_DIDENTERFOREGROUND, type::app_did_enter_foreground);
  ASSERT_EQ(SDL_DISPLAYEVENT, type::display);
  ASSERT_EQ(SDL_WINDOWEVENT, type::window);
  ASSERT_EQ(SDL_SYSWMEVENT, type::system);
  ASSERT_EQ(SDL_KEYDOWN, type::key_down);
  ASSERT_EQ(SDL_KEYUP, type::key_up);
  ASSERT_EQ(SDL_TEXTEDITING, type::text_editing);
  ASSERT_EQ(SDL_TEXTINPUT, type::text_input);
  ASSERT_EQ(SDL_KEYMAPCHANGED, type::keymap_changed);
  ASSERT_EQ(SDL_MOUSEMOTION, type::mouse_motion);
  ASSERT_EQ(SDL_MOUSEBUTTONDOWN, type::mouse_button_down);
  ASSERT_EQ(SDL_MOUSEBUTTONUP, type::mouse_button_up);
  ASSERT_EQ(SDL_MOUSEWHEEL, type::mouse_wheel);
  ASSERT_EQ(SDL_JOYAXISMOTION, type::joystick_axis_motion);
  ASSERT_EQ(SDL_JOYBALLMOTION, type::joystick_ball_motion);
  ASSERT_EQ(SDL_JOYHATMOTION, type::joystick_hat_motion);
  ASSERT_EQ(SDL_JOYBUTTONDOWN, type::joystick_button_down);
  ASSERT_EQ(SDL_JOYBUTTONUP, type::joystick_button_up);
  ASSERT_EQ(SDL_JOYDEVICEADDED, type::joystick_device_added);
  ASSERT_EQ(SDL_JOYDEVICEREMOVED, type::joystick_device_removed);
  ASSERT_EQ(SDL_CONTROLLERAXISMOTION, type::controller_axis_motion);
  ASSERT_EQ(SDL_CONTROLLERBUTTONDOWN, type::controller_button_down);
  ASSERT_EQ(SDL_CONTROLLERBUTTONUP, type::controller_button_up);
  ASSERT_EQ(SDL_CONTROLLERDEVICEADDED, type::controller_device_added);
  ASSERT_EQ(SDL_CONTROLLERDEVICEREMOVED, type::controller_device_removed);
  ASSERT_EQ(SDL_CONTROLLERDEVICEREMAPPED, type::controller_device_remapped);
  ASSERT_EQ(SDL_FINGERDOWN, type::touch_down);
  ASSERT_EQ(SDL_FINGERUP, type::touch_up);
  ASSERT_EQ(SDL_FINGERMOTION, type::touch_motion);
  ASSERT_EQ(SDL_DOLLARGESTURE, type::dollar_gesture);
  ASSERT_EQ(SDL_DOLLARRECORD, type::dollar_record);
  ASSERT_EQ(SDL_MULTIGESTURE, type::multi_gesture);
  ASSERT_EQ(SDL_CLIPBOARDUPDATE, type::clipboard_update);
  ASSERT_EQ(SDL_DROPFILE, type::drop_file);
  ASSERT_EQ(SDL_DROPTEXT, type::drop_text);
  ASSERT_EQ(SDL_DROPBEGIN, type::drop_begin);
  ASSERT_EQ(SDL_DROPCOMPLETE, type::drop_complete);
  ASSERT_EQ(SDL_AUDIODEVICEADDED, type::audio_device_added);
  ASSERT_EQ(SDL_AUDIODEVICEREMOVED, type::audio_device_removed);
  ASSERT_EQ(SDL_SENSORUPDATE, type::sensor_update);
  ASSERT_EQ(SDL_RENDER_TARGETS_RESET, type::render_targets_reset);
  ASSERT_EQ(SDL_RENDER_DEVICE_RESET, type::render_device_reset);
  ASSERT_EQ(SDL_USEREVENT, type::user);
