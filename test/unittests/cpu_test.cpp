...
{
  CHECK(cen::cpu::has_rdtsc() == SDL_HasRDTSC());
}

TEST_CASE("cpu::has_altivec", "[cpu]")
{
  CHECK(cen::cpu::has_altivec() == SDL_HasAltiVec());
}

TEST_CASE("cpu::has_mmx", "[cpu]")
{
  CHECK(cen::cpu::has_mmx() == SDL_HasMMX());
}

TEST_CASE("cpu::has_3dnow", "[cpu]")
{
  CHECK(cen::cpu::has_3dnow() == SDL_Has3DNow());
}

TEST_CASE("cpu::has_sse", "[cpu]")
{
  CHECK(cen::cpu::has_sse() == SDL_HasSSE());
}

TEST_CASE("cpu::has_sse2", "[cpu]")
{
  CHECK(cen::cpu::has_sse2() == SDL_HasSSE2());
}

TEST_CASE("cpu::has_sse3", "[cpu]")
{
  CHECK(cen::cpu::has_sse3() == SDL_HasSSE3());
}

TEST_CASE("cpu::has_sse41", "[cpu]")
{
  CHECK(cen::cpu::has_sse41() == SDL_HasSSE41());
}

TEST_CASE("cpu::has_sse42", "[cpu]")
{
  CHECK(cen::cpu::has_sse42() == SDL_HasSSE42());
}

TEST_CASE("cpu::has_avx", "[cpu]")
{
  CHECK(cen::cpu::has_avx() == SDL_HasAVX());
}

TEST_CASE("cpu::has_avx2", "[cpu]")
{
  CHECK(cen::cpu::has_avx2() == SDL_HasAVX2());
}

TEST_CASE("cpu::has_avx512f", "[cpu]")
{
  CHECK(cen::cpu::has_avx512f() == SDL_HasAVX512F());
}

TEST_CASE("cpu::has_neon", "[cpu]")
{
  CHECK(cen::cpu::has_neon() == SDL_HasNEON());
}

TEST_CASE("cpu::has_arm_simd", "[cpu]")
{
  CHECK(cen::cpu::has_arm_simd() == SDL_HasARMSIMD());
}
