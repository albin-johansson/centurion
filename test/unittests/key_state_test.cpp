...
{
  CHECK(ctn::key_state::unique());
  CHECK(ctn::key_state::shared());
}
...
TEST_CASE("key_state::is_pressed", "[key_state]")
{
  ...
  CHECK(!state.is_pressed(ctn::scan_code{-1}));
  CHECK(!state.is_pressed(SDL_NUM_SCANCODES));
  CHECK(!state.is_pressed(ctn::scan_code{SDL_NUM_SCANCODES + 1}));
}
...
TEST_CASE("key_state::is_held", "[key_state]")
{
  ...
  CHECK(!state.is_held(ctn::scan_code{-1}));
  CHECK(!state.is_held(SDL_NUM_SCANCODES));
  CHECK(!state.is_held(ctn::scan_code{SDL_NUM_SCANCODES + 1}));
}
...
TEST_CASE("key_state::was_just_pressed", "[key_state]")
{
  ...
  CHECK(!state.was_just_pressed(ctn::scan_code{-1}));
  CHECK(!state.was_just_pressed(ctn::scan_code{SDL_NUM_SCANCODES + 1}));
}
...
TEST_CASE("key_state::was_just_released", "[key_state]")
{
  ...
  CHECK(!state.was_just_released(ctn::scan_code{-1}));
  CHECK(!state.was_just_released(ctn::scan_code{SDL_NUM_SCANCODES}));
  CHECK(!state.was_just_released(ctn::scan_code{SDL_NUM_SCANCODES + 1}));
}
...
TEST_CASE("key_state::modifier_active", "[key_state]")
{
  ...
  ctn::key_state state;
  CHECK(!state.modifier_active(ctn::key_modifier::caps));
